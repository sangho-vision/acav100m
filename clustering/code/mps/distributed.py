...
def node_selection(full_urls, index=None, total=None, is_train=False, no_str_ok=False):
    ...
    if index == 0 and total > len(full_urls):
        warnings.warn(f"num_processes {total} > num_shards {len(full_urls)}")
    if is_train:
